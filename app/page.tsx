...
function LoginButton({ className = "" }: { className?: string }) {
  return (
    <Link
      href="/login"
      className={`inline-flex items-center justify-center rounded-xl bg-blue-600 px-8 py-3.5 text-sm font-bold text-white transition hover:bg-blue-500 ${className}`}>
      Log ind
    </Link>
  );
}
...
function Section({
  ...
  return (
    <section className="rounded-2xl border border-[#1e2d45] bg-[#0f1520] p-6 sm:p-8">
      {label && (
        <p className="mb-1 text-xs font-semibold uppercase tracking-widest text-blue-400/80">
          {label}
        </p>
      )}
      {title && (
  ...
  return (
    <div className="min-h-screen bg-[#080c14] text-slate-200">
      <div className="pointer-events-none fixed inset-0 flex items-start justify-center">
        <div className="mt-[-10%] h-[500px] w-[500px] rounded-full bg-blue-600/5 blur-[150px]" />
      </div>
      ...
      <main className="relative mx-auto max-w-lg px-5 pb-12 pt-10 sm:max-w-xl sm:pt-16">
        ...
        <header className="mb-10 text-center sm:mb-14">
          <div className="mb-6 inline-flex items-center gap-2">
            ...
            <span className="text-xl font-black tracking-widest text-blue-400">ZYFLEX</span>
          </div>
          ...
          <p className="mx-auto mt-4 max-w-md text-sm leading-relaxed text-slate-400 sm:text-base">
            Zyflex viser din chauffør hvor der er gang i byen lige nu — efter tidspunkt, zoner og
        ...
        <div className="space-y-4 sm:space-y-5">
          ...
          <Section title="Hvad Zyflex ikke er">
        ...
        <footer className="mt-12 text-center sm:mt-16">
          <LoginButton />
          <p className="mt-6 text-xs text-slate-600">Zyflex ApS</p>
        </footer>
      </main>
    </div>
  );
